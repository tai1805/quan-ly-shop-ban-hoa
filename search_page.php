...
$user_id = $_SESSION['user_id']; // Lấy user_id từ phiên đang hoạt động

if(!isset($user_id)){
   header('location:login.php'); // Nếu không có user_id, chuyển hướng đến trang đăng nhập
};
...
if(isset($_POST['add_to_wishlist'])){
   ...
   // Nếu sản phẩm chưa có trong danh sách yêu thích hoặc giỏ hàng, thêm vào danh sách yêu thích
...
if(isset($_POST['add_to_cart'])){
   ...
   if($check_cart_numbers->rowCount() > 0){
      ...
      if($check_wishlist_numbers->rowCount() > 0){
          // Nếu sản phẩm đã có trong danh sách yêu thích, xóa khỏi danh sách yêu thích
...
<?php include 'header.php'; //trang tiêu đề  ?> 

<section class="search-form">

   <form action="" method="POST">
      <input type="text" class="box" name="search_box" placeholder="Nhập sản phẩm muốn tìm...">
