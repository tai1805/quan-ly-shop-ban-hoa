<?php
//hiển thị các sản phẩm của một loại cụ thể
@include 'config.php';
...
if(isset($_POST['add_to_wishlist'])){
   //trích dẫn thông tin sản phẩm và thêm vào yêu thích
   $pid = $_POST['pid'];
...
if(isset($_POST['add_to_cart'])){
   // trích dẫn thông tin sản phẩm và thêm vào giỏ hàng
   $pid = $_POST['pid'];
   $pid = filter_var($pid, FILTER_SANITIZE_STRING);
   $p_name = $_POST['p_name'];
   $p_name = filter_var($p_name, FILTER_SANITIZE_STRING);
...
<section class="products">
   <!-- Hiển thị danh sách sản phẩm của loại sản phẩm được chọn -->

   <h1 class="title">các loại sản phẩm</h1>
